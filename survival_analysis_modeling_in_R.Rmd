...
We can use the `ggadjustedcurves()` function from the {survminer} package to plot _adjusted survival curves_ from our model.
...
Another way to visualize the model is to estimate survival proportions for _particular risk groups_. We can do that with the `survfit()` function. First we specify the risk groups in a new data frame. Below we specify two groups: a treated group with median logwbc and placebo group with median logwbc. We then call `ggsurvplot()` on the survfit object.

```{r}
nd <- data.frame(rx = c("placebo","trt"), logwbc = median(d$logwbc))
fit <- survfit(m2, newdata = nd)
ggsurvplot(fit, data = nd, conf.int = TRUE)
```

Another example: treatment risk groups with different logwbc levels. Notice the `ggsurvplot()` offers a `legend.labs` argument to change the legend labels. We set the confident interval off for demo purposes, but setting it to TRUE reveals the enormouse uncertainty in these estimates.

```{r}
nd <- data.frame(rx = "trt", logwbc = c(2.5,3,3.5))
fit <- survfit(m2, newdata = nd)
ggsurvplot(fit, data = nd, conf.int = FALSE,
...
fit <- survfit(whas_m2, newdata = nd)
ggsurvplot(fit, data = nd, conf.int = TRUE,
           legend.labs = nd$age)
```
...
Again we can plot adjusted survival curves for the variables we modeled.

```{r}
ggadjustedcurves(m4, variable = "rx", data = d)
```

We can also plot adjusted survival curves for the stratifying variable.

```{r}
ggadjustedcurves(m4, variable = "sex", data = d)
```

Notice this isn't much different from what we get when plot a Kaplan-Meier curve for the raw data by sex. We only stratified on sex in the model above, we didn't use it as a predictor.

```{r}
survfit(Surv(survt, status) ~ sex, data = d) |> ggsurvplot()
```
...
In either case we need to extend the Cox PH model to incorporate time. Recall the standard Cox PH model. Notice time is not part of the linear predictor (XB). It's only part of the baseline hazard which it doesn't estimate.
...
To illustrate we'll use the veteran data set that comes with the {survival} package, which has data on a randomized trial of two treatment regimens for lung cancer. Below we model survival time as a function treatment (1=standard, 2=test) and Karnofsky performance score (0 - 100), which is a scale that measures a patient's ability to carry out activities of daily living. Higher score is better. The data has information on 137 subjects.
...
The correct way is to use a _time-transform_ function via the `tt` argument. The usual syntax is `function(x, t, ...) x * t` where `x` is the argument for the predictor, `t` is the argument for `time`, and `...` is the "dots" argument used to pass through other arguments from other functions. Below we log transform the `t` argument since time is heavily skewed.
