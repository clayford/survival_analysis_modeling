...
Below we show the expansion for the first four subjects. Subjects 85 and 77 were the first to experience the event. Subject 95 was the second, so they get two records. Subject 53 was the third, so they get three records, and so on. (These are row numbers in the original data set.)

```{r}
X <- cbind(tdm2$x, subj = trunc(as.numeric(rownames(tdm2$x)))) |>
  as.data.frame()
View(subset(X, subj %in% c(85,77,95,53)))
```

If we want, we can expand the data ourselves using the `survSplit()` function from the {survival} package. Now we re-fit both models to the expanded data. This time it's OK to put "time" on the right-hand side of the model. You may wish to verify these model results match the previous fits.

```{r}
dtimes <- unique(veteran$time[veteran$status == 1])
veteran_long <- survSplit(Surv(time, status) ~ ., veteran, 
                    cut = dtimes)
...
```{r}
tdm2 <- coxph(Surv(start, stop, event) ~ transplant + surgery + tt(age),
             data= jasa1, tt = function(x,t,...)x*log(t))
summary(tdm2)
```
