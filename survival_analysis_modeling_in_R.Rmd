...
We're going to use the following packages in this workshop. The {survival} package is a recommended R package and comes with R. The {survminer} package is a user-contributed package that needs to be installed.
...
The `ggsurvplot()` function from the {survminer} package looks nicer and includes a legend to identify groups.
...
As before we can use the `summary()` method on the survfit object to see the specific survival probabilities at specific times.

```{r}
summary(sfit1, times = c(5,10,15,20))
```

Likewise we can print the survfit object to see estimated median survival times.

```{r}
sfit1
```

See the section "How survfit() calculates covariate-adjusted survival curves" in the appendix of this document for details on how the curves are calculated.
...
See the section "Adjusted Survival Curves for Cox Proportional Hazards Model" in the appendix of this document for an alternative but computationally more intensive approach to creating adjusted survival curves.
...
The {survminer} package offers the `ggcoxdiagnostics()` function to create similar plots, but is currently (as of Oct 2023) throwing warnings about deprecated tidyverse functions. It also creates the same type of plot regardless whether the predictor is numeric or categorical.
...
### How basehaz() calculates cumulative hazards

Above we used `basehaz()` to calculate cumulative hazards on our way to estimating covariate-adjusted survival curves. How does that work?

Calculating cumulative hazards from a Cox PH model is tricky, especially when you have ties in the data. We demonstrate below assuming we have used the breslow method to handle ties. This is easier to show. First we refit model m2 using `ties = "breslow"`. (Recall the default method to handle ties is "efron"). The `basehaz()` returns a data frame with two columns: cumulative hazards and time. 

```{r}
m2 <- coxph(Surv(survt, status) ~ logwbc + rx, data = d, 
            ties = "breslow")
bh <- basehaz(m2)
bh
```

Here's one way to calculate the cumulative hazards by hand. The code below implements the following function to calculate the time-specific individual hazards, which are saved into a vector called "haz".

$$h_0(t_i) = \frac{d_i}{\sum\limits_{i\in{R_i}}\text{exp}(z_i\hat{\beta})} $$ 

We then use the `cumsum()` function to calculate the cumulate hazards.

```{r}
# get the model coefficients
beta <- coef(m2)
# make prediction with model for all data and center
# Note: coxph() internally scales and centers data
xcenter <- sum(m2$means * beta)
risk <- c(exp(model.matrix(m2) %*% beta - xcenter))
# get the time vector (includes the censored times)
dtime <- d$survt
# get the censoring vector
status <- d$status
# T/F vector of events (TRUE = time when someone experienced event)
death <- status == 1
# get events in order of occurrence
nevent <- as.vector(rowsum(as.numeric(death), dtime))
# cumulative risk (in order of occurrence)
rcumsum <- function(x) rev(cumsum(rev(x)))
nrisk <- rcumsum(rowsum(risk, dtime))
# calculate estimated hazards; this is h0(t)
haz <- nevent/nrisk 
# calculate cumulative hazard; basehaz() does this!
cumhaz <- cumsum(haz)
# compare to basehaz() output above
cbind(cumhaz, time = sort(unique(dtime)))
```


### Adjusted Survival Curves for Cox Proportional Hazards Model

In this workshop we presented covariate-adjusted survival curves using a fitted Cox PH model via `survfit()`. Another approach is to produce a predicted survival curve _for every subject_ of some hypothetical population, and then _take the average of these curves_. This approach is implemented in the `survexp()` function.

Recall the Cox PH model m2:

```{r}
formula(m2)
```

An example of covariate-adjusted survival curves is two curves, one each for rx level with logwbc set to the overall median value. Below `survfit()` calculates two curves based on the model coefficients and the specified covariates.

```{r}
nd <- data.frame(rx = c("trt","placebo"), logwbc = median(d$logwbc))
sf1 <- survfit(m2, newdata = nd)
plot(sf1, lty = 1:2)
```

Compare this to the adjusted survival curves returned by `survexp()`. Below we specify we want survival curves for each level of rx using the formula notation, `~ rx`. We set `ratetable = m2` to reference our fitted Cox model. Below `survexp()` calculates survival curves for _each subject_ as if they were in the placebo group and then calculates the average survival curve (dashed line), and then does the same for everyone as if they were in the treated group (solid line). 

```{r}
sf2 <- survexp(~ rx, data=d, ratetable=m2)
plot(sf2, lty = 1:2)
```
...
plot(sf2, lty = 1:2, main = "survexp()")
par(op)
```

The one on the left shows two expected curves (placebo and trt) when logwbc is held at the median value, while the one on the right shows two average curves (placebo and trt) that were calculated from subject-specific survival curves that were estimated using subject-specific logwbc values. 

The {survminer} function `ggadjustedcurves()` creates these types of adjusted survival curves. Be warned this function can be quite slow for large data sets that have thousands of subjects.

See the "Adjusted Survival Curves" vignette that accompanies the {survival} package for more information.

```{r}
vignette("adjcurve", package = "survival")
```
